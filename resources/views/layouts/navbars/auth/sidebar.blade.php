<aside data-color="info"
    ...
    <div class="collapse navbar-collapse w-auto bg-gray-200" id="sidenav-collapse-main">
        <ul class="navbar-nav">
            ...
            @php
                $isQuoteRoute = Route::currentRouteNamed('quote-requests.*') || Route::currentRouteNamed('part-quotes.*');
            @endphp

            <li class="nav-item pb-2">
                <a class="nav-link {{ ($isQuoteRoute ? 'active' : '') }}"
                   href="{{ route('quote-requests.index') }}">
                    <div
                        class="icon icon-shape icon-sm shadow border-radius-md bg-white text-center me-2 d-flex align-items-center justify-content-center">
                        <i style="font-size: 1rem;"
                           class="fas fa-lg fa-car ps-2 pe-2 text-center text-dark {{ ($isQuoteRoute ? 'text-white' : 'text-dark') }} "
                           aria-hidden="true"></i>
                    </div>
                    <span class="nav-link-text ms-1">Quotes</span>
                </a>

                @if($isQuoteRoute ? 'active' : '')
                    <ul class="nav ms-4 ps-3 mt-3">
                        <li class="nav-item ">
